...
static long getOffset(long x, long y, long z, long tx, long ty) {
...
Image<T>::Image(const TSize *size) {
  ...
  try {
    ...
    std::cerr
        << "Image<T>::Image(const TSize *size) : could not allocate buffer : "
        << e.what() << std::endl;
    exit(-1);
  }
}
...
Image<T>::Image(const TSize xSize, const TSize ySize, const TSize zSize) {
  ...
  try {
    ...
    std::cerr << "Image<T>::Image(TSize xSize, TSize ySize, TSize zSize) : "
...
Image<T>::Image(const TSize *size, const TSpacing *spacing, const T *data) {
  ...
  try {
    ...
    std::cerr << "Image<T>::Image(const TSize *size, const TSpacing *spacing, "
                 "const T *data) : could not allocate buffer : "
              << e.what() << std::endl;
    exit(-1);
  }
...
Image<T>::Image(const Image<T> &im) {
  ...
  try {
    ...
    std::cerr
        << "Image<T>::Image(const Image<T> &im) : could not allocate buffer : "
        << e.what() << std::endl;
    exit(-1);
  }

  for (long i = 0; i < this->dataSize; i++) data[i] = im.data[i];
}
...
Image<T> &Image<T>::operator=(const Image<T> &im) {
  if (this != &im) {
    ...
    try {
      ...
      std::cerr << "Image <T> & Image<T>::operator=(const Image <T> &im) : "
                   "could not allocate buffer : "
                << e.what() << std::endl;
      exit(-1);
    }

    for (long i = 0; i < this->dataSize; i++) this->data[i] = im.data[i];
  }
  return *this;
}
...
Image<T>::Image(const Image<T2> &im) {
  ...
  try {
    ...
    std::cerr << "Image<T>::Image( Image<T2> &im): could not allocate buffer : "
              << e.what() << std::endl;
    exit(-1);
  }
...
Image<T> &Image<T>::operator+=(Image<T> &op) {
  for (long i = 0; i < dataSize; i++) this->data[i] += op(i);
  return *this;
}
...
void Image<T>::fill(const T value) {
  for (long i = 0; i < this->dataSize; i++) this->data[i] = value;
}
...
template <class VoxelType>
void Image<VoxelType>::copy(Image<VoxelType> &im, TCoord x1, TCoord y1,
                            TCoord z1, TCoord x2, TCoord y2, TCoord z2,
                            TCoord px, TCoord py, TCoord pz) {
  for (TCoord x = 0; x <= x2 - x1; x++) {
    for (TCoord y = 0; y <= y2 - y1; y++) {
      for (TCoord z = 0; z <= z2 - z1; z++) {
...
template <class VoxelType>
void Image<VoxelType>::copyFast(Image<VoxelType> &im, long x1, long y1, long z1,
                                long x2, long y2, long z2, long px, long py,
                                long pz) {
  typename Image<VoxelType>::iteratorXYZ it;
  typename Image<VoxelType>::iteratorXYZ end = this->end();
  for (it = this->begin(); it != end; ++it)
    this->operator()(px + it.x, py + it.y, pz + it.z) =
        im(it.x + x1, it.y + y1, it.z + z1);
}

template <class VoxelType>
void Image<VoxelType>::copyFast(Image<VoxelType> &im, TCoord px, TCoord py,
                                TCoord pz) {
  TSize *taille = im.getSize();
  copyFast(im, 0, 0, 0, taille[0] - 1, taille[1] - 1, taille[2] - 1, px, py,
           pz);
}

template <class VoxelType>
void Image<VoxelType>::copy(Image<VoxelType> &im, TCoord px, TCoord py,
                            TCoord pz) {
  ...
  for (long x = 0; x < taille[0]; x++) {
    for (long y = 0; y < taille[1]; y++) {
      for (long z = 0; z < taille[2]; z++) {
        im(x, y, z) =
            im(taille[0] - x - 1, taille[1] - y - 1, taille[2] - z - 1);
      }
    }
  }
...
bool Image<VoxelType>::isPosValid(TCoord x, TCoord y, TCoord z) const {
  return ((x >= 0) && (x < size[0]) && (y >= 0) && (y < size[1]) && (z >= 0) &&
          (z < size[2]));
}
