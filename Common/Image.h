 ...
 public:
  ...
  void setSize(const TSize *size) {
    ...
    try {
      ...
      std::cerr << "Image::Image(...) : could not allocate buffer : "
                << e.what() << std::endl;
      exit(-1);
    }
  }
  ...
  void setSpacing(const TSpacing &vx = 1.0, const TSpacing &vy = 1.0,
                  const TSpacing &vz = 1.0) {
    spacing[0] = vx;
    spacing[1] = vy;
    spacing[2] = vz;
  }
  ...
  bool operator==(Image<T> &op) {
    ...
    for (it = this->begin(); it != end; ++it, ++itOp)
      if (*it != *itOp) return false;
    return true;
  }

  bool operator!=(Image<T> &op) { return !operator==(op); }
  ...
  void copy(Image<T> &im, TCoord x1, TCoord y1, TCoord z1, TCoord x2, TCoord y2,
            TCoord z2, TCoord px, TCoord py, TCoord pz);

  void copyFast(Image<T> &im, long x1, long y1, long z1, long x2, long y2,
                long z2, long px, long py, long pz);
  void copyFast(Image<T> &im, TCoord px, TCoord py, TCoord pz);
  void copy(Image<T> &im, TCoord px, TCoord py, TCoord pz);

  void enlarge();

  long getOffset(TCoord x, TCoord y = 0, TCoord z = 0) {
    return x + y * size[0] + z * size[0] * size[1];
  }

  long getOffset(Point<TCoord> p) {
...
Image<T> operator+(Image<T> &a, T s) {
  Image<T> res = a;
  for (long i = 0; i < a.getBufSize(); i++) res(i) += s;
  return res;
}
