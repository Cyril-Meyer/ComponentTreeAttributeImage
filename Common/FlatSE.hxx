...
inline size_t FlatSE::getNbPoints() const { return points.size(); }
...
inline void FlatSE::setContext(const TSize *imSize) {
  ...
  for (it = points.begin(); it != end; ++it) {
    TSize offset =
        it->x + (it->y) * imSize[0] + (it->z) * imSize[0] * imSize[1];
...
/*! Convert structuring element into GImage. The center of image represents the
 * origin.
 */
...
inline void FlatSE::makeSymmetric() {
  ...
  for (std::vector<Point<TCoord> >::iterator it = this->points.begin();
...
inline void FlatSE::make3DAxialSegment(int l) {
  ...
  for (int i = -l; i <= l; i++) points.push_back(Point<TCoord>(0, 0, i));
  this->setNegPosOffsets();
}
...
void FlatSE::makeCircle2D(Image<T> &img, double r, double t) {
  ...
  for (int x = -ux; x <= ux; x++)
    for (int y = -uy; y <= uy; y++)
      if (x * x * vx * vx + y * y * vy * vy <= rrtt &&
          x * x * vx * vx + y * y * vy * vy >= rr) {
        Point<TCoord> p;
        p.x = x;
        p.y = y;
        p.z = 0;
